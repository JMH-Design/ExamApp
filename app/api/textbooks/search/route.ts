...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Limit to 10 as per plan
    const limited = items.slice(0, 10);

    // Enrich with Google Books (cover, year)
    const textbooks: TextbookSearchResult[] = await Promise.all(
    ...
    console.error("Textbook search error:", err);
    return NextResponse.json(
